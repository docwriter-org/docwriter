...
Every file in your workspace folder. Click any file to open it as a tab in
the editor. There is no special "supported file types" list. Any text
...
1. **Table of contents.** Headings from the active tab, generated on the
   fly. Click a heading to jump.
2. **Pending edit cards.** When the agent proposes a change, a card
   appears here with Accept and Reject buttons. See [Reviewing
   edits](/agent/reviewing-edits).
...
At the top of the pane is the **agent pill**, a small mascot icon. The
mascot animates while the agent is running. Clicking the pill wakes the
agent on demand (the auto-wake usually does this for you, but the pill
is the manual override).

Below the pill, every interaction the agent has with your workspace is
logged as a card:
