...
The same pane is where [pending agent
reviews](/agent/reviewing-edits) appear, so for a tab with
in-flight agent edits you'll see both the TOC (top) and the pending
review cards (below).
