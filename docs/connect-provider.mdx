...
When you switch to an earlier session through **Settings**, then **Sessions**, DocWriter restores its saved provider and model when the provider session is still valid. Read [Sessions and history](/agent/sessions-and-history) when an old provider session cannot resume.
