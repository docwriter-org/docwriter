...
DocWriter has three ways to give the agent guidance that lasts beyond one request. Choose a type based on whether you need a standing instruction, material to consult, or a repeatable procedure.
...
- Use a **rule** for a short instruction, such as a required term or citation format. The agent applies enabled rules to later requests.
...
Writing rules appear in the toolbar above the editor.
