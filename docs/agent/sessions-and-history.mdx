...
The expanded agent dock shows the current turn, tool progress, subagent progress, errors, and the running session cost. After the first round, its cost detail includes input, output, and cache token counts with the estimated cost in US dollars.
...
Open **Settings**, then **Sessions**. Search earlier sessions and switch to one. DocWriter restores its provider, model, and saved conversation when possible.
