...
Each card shows the diff and has its own Accept, Reject, and
Retry-with-feedback buttons. You must accept or reject the oldest card
before the next one becomes actionable (FIFO order).
