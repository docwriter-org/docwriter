...
Run the hook once. The template builds the entry file, updates the bibliography and references, and writes the PDF named in **Output**. You should see **done** status with an exit code of zero, and the PDF at that output path. Fix any missing TeX packages or file names shown in a failed hook result.
