...
The supervisor owns sign-in. Done, three modes behind `SUPERVISOR_AUTH`:

- **`clerk`** (default): Clerk's hosted sign-in UI, so participants without a GitHub account can use email, a magic link, or Google. Clerk is used only at sign-in: the page posts one Clerk session token to the supervisor, which verifies it with `@clerk/backend`, reads the primary email, checks the allowlist by email, and issues its own thirty-day cookie. That keeps ClerkJS out of the app page, where Clerk's sixty-second tokens could not be refreshed. The user id is `clerk:<user id>`, so an email change keeps the workspace.
- **`github`**: a plain OAuth app with the `read:user` scope, no vendor. The user id is `gh:<numeric id>`.
- **`dev`**: any name, for laptops and tests.

All three end in the same signed session cookie, allowlist file, and sign-out route. The app processes never see any of this; they trust the gateway header, which only the supervisor sends.

## Phase 4: machine and deploys

Two targets share the supervisor and the backup layer. Both are written; neither has run on a real host yet.

**Fly.io, the primary target** (`deploy/fly/`). One Fly Machine is a Firecracker VM, so the process-per-user design and bubblewrap work unchanged, and the pieces already set up for PR #51 carry over: the `docwriter-app` name, the `app.docwriter.org` certificate, the deploy token, and the production Clerk instance.

1. `Dockerfile`: the production build plus bubblewrap, Litestream, and rclone, on the glibc Node image. The whole app tree is world-readable because every user uid binds it read-only.
2. `entrypoint.sh`: prepares `/data`, self-tests bubblewrap as an unprivileged uid and refuses to start if that fails, starts Litestream and the hourly backup loop when their secrets are set, then runs the supervisor.
3. `fly.toml`: one machine that Fly never stops for idleness, a 32 GB `performance-4x` VM, the volume at `/data`, a public liveness check, a connection concurrency limit high enough for one WebSocket per open tab (Fly's default of 25 would refuse the twenty-sixth user), and a 90 second stop timeout so every user process can flush.
4. Fly's proxy terminates TLS, so there is no Caddy. The supervisor listens on the machine's private address.
5. A deploy replaces the machine, so running user processes stop and browsers reconnect. Acceptable for a study. Rollback is `fly deploy --image` with the previous image.

**A plain VM, the alternative** (`deploy/machine/`), for when a bigger box for less money matters more than reusing Fly: `provision.sh` for Ubuntu 24.04, Caddy for TLS, a systemd unit with cgroup delegation, release directories with a symlink so deploys keep sessions, and the same backup layer through systemd units.

`.github/workflows/deploy-hosted.yml` deploys on push to `main` to whichever target the repository variable `HOSTED_DEPLOY_TARGET` names.
...
| 3 Auth | done: Clerk, GitHub, and dev modes, tested with fakes | sign-in with an allowlist |
| 4 Machine and deploys | Fly image and VM files written; entrypoint exercised locally; not yet run on a host | the domain serving `main` |
