...
Recommendation: start here. The supervisor is the only new code, the image and entrypoint are shared with the machine-per-user design, and the switch to machines later is contained in the spawner.
