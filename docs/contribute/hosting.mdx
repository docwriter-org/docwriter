...
- **The network namespace stays shared**, so every process can reach every localhost port. Each process therefore gets its own gateway secret (`DOCWRITER_GATEWAY_SECRET`) and refuses any request or WebSocket handshake without it; only the supervisor knows it.
...
Recommendation: start here. The supervisor is the only new code, the image and entrypoint are shared with the machine-per-user design, and the switch to machines later is contained in the spawner. The big machine can be a Fly Machine, which is a VM and keeps the certificate, app, and Clerk setup made for the earlier attempt, or a plain VM from anyone. The step-by-step version is on the [Hosting plan](/contribute/hosting-plan) page, and the implementation lives under `deploy/` with its runbook in `deploy/README.md`.
