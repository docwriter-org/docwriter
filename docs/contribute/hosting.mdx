...
- **Volumes are backed up.** A volume lives on one host. Keep the platform's daily snapshots on, and add a periodic copy of each workspace's markdown and SQLite file to object storage so a lost host is an inconvenience, not lost writing.
