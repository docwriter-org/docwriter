...
Start DocWriter with two browser windows open on the same file. Type in one window and confirm that the text appears in the other. Propose an agent edit and confirm that both windows show the pending review while the workspace file still contains the accepted text. Accept the edit in one window and confirm that both windows and the workspace file receive the accepted text.

Then reject a separate proposal and confirm that the accepted text does not change. Run `npm run check` and `npm run build` after the manual checks. Successful verification means that every client converges on the same text, and no temporary document or file write bypasses the review state.
