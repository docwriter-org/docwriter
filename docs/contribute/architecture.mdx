...
When the agent proposes an edit, `edit_doc` writes tracked changes into the live document: `insertion` and `deletion` text marks, and a paragraph attribute for whole lines added or removed, each carrying the id of the comment thread the proposal belongs to (`src/lib/shared/proposals.ts`). The committed text does not change yet; the browser renders the marks with CSS. A passage has one thread, and a new proposal on a thread replaces its old one.

When the user accepts, the server keeps the inserted text (marked as AI-written), removes the struck text, and resolves the thread in one transaction. When the user rejects or dismisses, the server reverts the marks and resolves the thread.
...
Comments and proposals live inside the Yjs document: the thread messages in a map, the thread's position as marks on the text. The committed text, comments, and proposals therefore reach every connected browser through the same update stream. Saved files hold the committed text only.
...
- Document encoding, thread storage, and transaction origins are in `src/lib/shared/ydoc-codec.ts`; proposals as marks, the three text views, and accept / reject are in `src/lib/shared/proposals.ts`.
