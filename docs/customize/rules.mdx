...
That stores a `Freeze: …` rule, paints a teal dashed highlight on the
passage, and shows a lock chip in the gutter. The agent's `edit_doc`
tool rejects edits that overlap the frozen quote.

Unlock from the gutter chip or by removing the freeze rule in Writing
rules.

<Frame>
  ![A frozen paragraph highlighted in the editor with a Freeze for agent chip in the gutter](/images/freeze-passage.png)
</Frame>
