...
The first request contains the open tab paths and the changes since each tab was last seen. It does not include every full file body. The agent calls `read_doc` when it needs the full content of an open tab, and its workspace tools can inspect other files when the request requires them.
