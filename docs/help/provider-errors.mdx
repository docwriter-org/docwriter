...
Open **Settings**, then **Providers**. Check whether the selected provider shows **Connected · CLI login**, **Connected · API key**, or **Not connected**. Press **Refresh** after signing in from a terminal.
...
Codex can also use an existing command line login. Run `codex login` again in a terminal on this computer, then retry. DocWriter links `~/.codex/auth.json` into its per-workspace Codex home on the next request, so no restart is needed.
