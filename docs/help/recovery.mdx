...
A text tab paints after the first WebSocket sync. Check that the DocWriter server is still running and that its Hocuspocus port is available. Reload the browser after the server is healthy.
...
A proposal becomes stale when its source text changes. Reject the stale proposal or let DocWriter ask the agent to regenerate it against the current file.

## Deleted file

The tabs API removes tabs for files that no longer exist. Deleting a file inside DocWriter also deletes its saved Yjs update log so the content cannot reappear on reopen.
