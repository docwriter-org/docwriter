...
describe('formatListedThreads', () => {
	...
	it('lists only open threads by default and hints at dismissed ones', () => {
		const text = formatListedThreads('document.md', [open, dismissed], false, passages);
	...
	it('includes dismissed thread bodies when asked', () => {
		const text = formatListedThreads('document.md', [open, dismissed], true, passages);
	...
	it('still reports dismissed-only tabs without include_dismissed', () => {
		const text = formatListedThreads('document.md', [dismissed], false, passages);
