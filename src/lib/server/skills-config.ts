		...
		customSkills: Array.isArray(obj.customSkills)
			? obj.customSkills
					...
					.map((s) => ({
						...
						managedBy: typeof s.managedBy === 'string' ? s.managedBy : undefined,
...
/** Register or update a skill generated by DocWriter without creating a new
 * numbered skill on every regeneration. A user-installed skill is never
 * replaced. */
export function upsertManagedSkill(
	...
	path: string,
	managedBy: string
): { config: SkillConfig; id: string } {
	const config = readSkillsConfig();
	const normalizedId = sanitizeSkillId(preferredId);
	const existingManaged = config.customSkills.find((skill) => skill.managedBy === managedBy);
	let id = existingManaged?.id ?? normalizedId;
	const collision = config.customSkills.find((skill) => skill.id === id && skill.managedBy !== managedBy);
	if (collision || BUNDLED_SKILLS.some((skill) => skill.id === id)) {
		id = uniqueSkillId(`docwriter-${normalizedId}`, config);
	}
	const now = Date.now();
	const entry: CustomSkillConfig = {
		...
		enabled: true,
		managedBy,
		addedAt: existingManaged?.addedAt ?? now
	};
	config.customSkills = [
		...config.customSkills.filter((skill) => skill.managedBy !== managedBy),
		entry
	];
	writeSkillsConfig(config);
	syncSkillInstallations(config);
